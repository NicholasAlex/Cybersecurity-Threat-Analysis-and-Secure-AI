...
def holdout_split(samples, test_frac=0.2, seed=0):
    """
    One stratified train/test split, sample-level.

    LOSO is a fold per sample -- fine at HW5's n=5, infeasible at the public
    dataset's n=7,107 (that many federated simulations would never finish).
    At this scale a single held-out test set gives a stable estimate; splitting
    within each family (rather than globally) keeps rare families like Adware
    (379 samples) from landing disproportionately on one side.

    `seed` should stay FIXED across a whole n-seeds sweep so every seed is
    evaluated against the identical test set -- only model init and client
    partitioning should vary between seeds, not what they're scored against.
    """
    ...
    for fam in sorted(by_family):
        ...
        order = rng.permutation(len(fam_samples))
        n_test = max(1, round(len(fam_samples) * test_frac))
        for i, idx in enumerate(order):
            (test if i < n_test else train).append(fam_samples[idx])
