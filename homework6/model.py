...
def _make_optimizer(model, optimizer, lr):
    """
    Build the local optimiser.

    NOTE ON ADAM IN FEDERATED LEARNING (added for HW6): a fresh optimiser is
    constructed on every fit() call, because a Flower client holds no state
    across rounds. For Adam that means its first- and second-moment estimates
    are RESET every round, so each round restarts adaptive scaling from zero
    on a model that is no longer freshly initialised. Under a non-IID split
    this makes the training loss diverge -- HW5 observed it climbing from 0.63
    to over 5.8 across 40 rounds. Plain SGD with momentum carries no
    cross-round state to lose and converges cleanly (final round loss 0.19).

    Note the divergence is cosmetic: fixing it does NOT recover accuracy,
    because the non-IID failure is genuine client drift at 1-2 samples per
    client. But a diverging baseline makes the DP noise effect impossible to
    attribute, so every HW6 experiment should use SGD.

    Default stays "adam" so HW5's published numbers reproduce exactly.
    """
    if optimizer == "adam":
...
def train_epochs(model, loader, epochs=1, lr=1e-3, device="cpu", class_weights=None,
    ...
    `dpsgd`: optional dict enabling MECHANISM 3, sample-level DP-SGD, via
    Opacus (Abadi et al. 2016). Keys: max_grad_norm, noise_multiplier. When
    set, PER-EXAMPLE gradients are clipped and noised at every optimiser step,
    which protects individual training windows rather than whole clients.
    Left None for every other experiment.
    """
    ...
    if dpsgd:
        ...
        # make_private returns wrapped copies; the underlying parameters are
        # shared, so weights read back out of `model` afterwards are correct.
        model, opt, loader = PrivacyEngine(accountant="rdp").make_private(
            module=model, optimizer=opt, data_loader=loader,
            noise_multiplier=dpsgd["noise_multiplier"],
            max_grad_norm=dpsgd["max_grad_norm"],
        )
